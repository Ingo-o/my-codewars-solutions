...
const narcissistic = (num) => {
    const arrOfDigits = String(num).split('').map(str => Number(str));
    const comparisonNum = arrOfDigits.reduce((acc, num) => acc + (num ** arrOfDigits.length), 0);
    return comparisonNum === num;
}
